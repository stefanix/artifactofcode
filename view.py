...
def get_entry_page_list():
    """Return a list of the tags and their article counts"""
    # Attempt to get a memcache'd copy first
    page_list = memcache.get('entry_page_list')
    ...
    page_list = []
    query = blog.Post.all()
    query.filter('is_entry_page = ', True)
    query.order('-edit_date')
    # query.run()  # not needed when using default query
    page_alpha = None
    for page in query:
        if page.title == config.SETTINGS['page_name_to_be_always_top']:
            page_alpha = {'title': page.title, 
                          'url': page.get_absolute_url() }
        else:
            page_list.append({'title': page.title, 
                              'url': page.get_absolute_url() })
    if page_alpha:
        page_list.insert(0, page_alpha)

    memcache.set('entry_page_list', page_list)
    return page_list
...
class Page:
    ...
    def render(self, handler, template_file, template_values={}):
        ...
        entry_page_list = get_entry_page_list()

        values = {'archive_list': archive_list,
                  'tag_list': tag_list,
                  'page_list': entry_page_list,
